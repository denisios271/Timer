...
import { MainTimer } from "./stop-watch.interface";
...
export class TimeService {
  ...
  public get mainTimer$(): Observable<MainTimer> {
    return this.timer$.pipe(
      map((miliseconds: number): MainTimer => this.secondsToStopWatch(miliseconds))
    );
  }
  ...
  private secondsToStopWatch(ms: number): MainTimer {
    let rest = ms % 10;
